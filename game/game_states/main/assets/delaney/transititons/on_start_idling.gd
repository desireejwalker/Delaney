...
func is_valid(actor, blackboard: Blackboard):
	...
	if get_parent() is WalkState:
		...
		return actor.velocity.length() <= 1
	if get_parent() is LightRecoveryState:
		...
		return actor.velocity.length() <= 1 and not blackboard.get_value("light_recovery_active")
	if get_parent() is AttackState:
		...
		return actor.velocity.length() <= 1 and not Input.is_action_pressed("attack") and blackboard.get_value("launch_level") == 0
	if get_parent() is LaunchState:
		# this transition is valid if the launch level is -1
		# and the the player is not moving (or barely moving at all)
		return actor.velocity.length() <= 1 and blackboard.get_value("launch_level") == -1
	if get_parent() is LaunchRecoveryState:
		# this transition is valid if the player is not moving (or barely moving at all)
		# and the launch recovery is inactive
		return actor.velocity.length() <= 1 and not blackboard.get_value("launch_recovery_active")
	return false
