...
const INPUT_SECTION = &'input_settings'
const AUDIO_SECTION = &'audio_settings'
const VIDEO_SECTION = &'video_settings'
const GAME_SECTION = &'gameplay_settings'
const APPLICATION_SECTION = &'application_settings'
const CUSTOM_SECTION = &'custom_settings'

const FULLSCREEN = &'Fullscreen'
const SCREEN_MODE = &'screen_mode'
const SCREEN_RESOLUTION = &'screen_resolution'
const GAME_RESOLUTION = &'game_resolution'
const WORLD_RESOLUTION = &'world_resolution'
const V_SYNC = &'V-Sync'
...
static func set_screen_mode(value: int, window: Window) -> void:
	window.mode = value as Window.Mode
	if window.mode == Window.MODE_WINDOWED:
		var current_game_resolution = PlayerConfig.get_config(VIDEO_SECTION, GAME_RESOLUTION, window.size)
		set_game_resolution(current_game_resolution, window)
...
static func set_game_resolution(value : Vector2i, window : Window, update_config : bool = true) -> void:
	if value.x == 0 or value.y == 0:
		return
	window.size = value
	if update_config:
		PlayerConfig.set_config(VIDEO_SECTION, GAME_RESOLUTION, value)

static func set_world_resolution(value: Vector2i, update_config: bool = true) -> void:
	pass
...
static func get_screen_mode(window: Window) -> int:
	return PlayerConfig.get_config(VIDEO_SECTION, SCREEN_MODE, window.mode)
...
static func get_game_resolution(window : Window) -> Vector2i:
	var current_resolution : Vector2i = window.size
	return PlayerConfig.get_config(VIDEO_SECTION, GAME_RESOLUTION, current_resolution)

static func get_world_resolution(window: Window) -> Vector2i:
	var current_resolution : Vector2i = window.size
	return PlayerConfig.get_config(VIDEO_SECTION, WORLD_RESOLUTION, current_resolution)
...
static func _set_screen_mode_from_config(window: Window) -> int:
	var screen_mode = PlayerConfig.get_config(VIDEO_SECTION, SCREEN_MODE, window.mode)
	set_screen_mode(screen_mode, window)
	return screen_mode

static func set_anti_aliasing(mode: Viewport.MSAA) -> void:
	pass
...
static func set_video_from_config(window : Window) -> void:
	...
	var screen_mode = _set_screen_mode_from_config(window)
	if screen_mode == Window.MODE_WINDOWED:
		var current_game_resolution = PlayerConfig.get_config(VIDEO_SECTION, GAME_RESOLUTION, window.size)
		set_resolution(current_game_resolution, window)
