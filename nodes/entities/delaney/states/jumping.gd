...
func _on_update(delta: float, actor: Node, blackboard: Blackboard) -> void:
	...
	var speed = actor.get_entity_stats().get_agility() * 0.8
	var velocity = _handle_jumping(actor.velocity, direction, speed, delta)
...
func _handle_transition_events(actor: Node, blackboard: Blackboard) -> bool:
	if Input.is_action_just_pressed("dive"):
		get_parent().fire_event("jumping/on_dive")
		return true
	
	if actor.velocity.y < 0:
		get_parent().fire_event("jumping/on_start_falling")
		return true
...
func _handle_jump_force(current_velocity: Vector3) -> Vector3:
	var velocity = current_velocity + (Vector3.UP * FORCE)
	return velocity
