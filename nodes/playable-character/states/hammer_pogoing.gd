...
@export var horizontal_force: float = 10
@export var vertical_force: float = 7.5
@export var gravity: float = 9.8
@export var acceleration: float = 75
@export var speed: float = 15
@export var max_horizontal_velocity: float = 100
...
func _handle_long_jump_force(current_velocity: Vector3, direction: Vector3) -> Vector3:
	var horizontal_velocity = Vector3(
		...
		0,
		current_velocity.z + (direction.z * horizontal_force)).limit_length(max_horizontal_velocity)
